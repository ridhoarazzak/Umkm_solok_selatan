...
import { ShoppingCart, Wand2, Loader2, Heart, MessageCircle, BadgeCheck } from 'lucide-react';
...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  const handleBuy = () => {
    // Logic: Uses product specific number if available, otherwise fallback (though all should have numbers now)
  ...
  return (
    <div className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-transparent hover:border-gray-100 flex flex-col h-full relative">
      ...
      <div className="relative overflow-hidden h-72">
        <div className="absolute inset-0 bg-gray-200 animate-pulse" /> {/* Placeholder loading skeleton effect could go here */}
        <img 
          src={product.image} 
          alt={product.name} 
      ...
      <div className="p-7 flex flex-col flex-grow">
        <div className="flex justify-between items-baseline mb-3">
           <h3 className="text-xl font-bold text-gray-900 group-hover:text-solok-gold transition-colors line-clamp-1" title={product.name}>
             {product.name}
           </h3>
        </div>
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-50 mt-auto">
          <button 
            onClick={handleBuy}
            className="flex-1 bg-green-600 text-white py-3.5 rounded-xl text-sm font-bold hover:bg-green-700 hover:shadow-lg hover:shadow-green-200 transition-all flex justify-center items-center gap-2 group/btn"
          >
